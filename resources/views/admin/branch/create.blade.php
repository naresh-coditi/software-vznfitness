...
{{-- Breadcrum --}}
@push('breadcrum')
    <div class="flex items-center gap-4 px-4 border-b bg-white border-gray-300 pl-6 py-2 shadow-sm  text-xl font-bold -mt-6">
...
@section('main-section')
    <section class="w-full px-6 mt-6 flex items-center justify-center">
        <form action="{{ route(auth()->user()->roleName . 'branch.store') }}" method="post"
            class="block md:w-1/2 w-full px-2  space-y-2">
            @csrf
            <div class="md:flex items-center gap-6">
                ...
                {{-- Location --}}
                <div class="w-full">
                    <x-input-label for="location" :value="__('Location')" astrik />
            ...
            <div>
                ...
                <textarea required name="address" id="address" cols="10" rows="5"
                    class="block mt-1 w-full border-gray-300 focus:border-orange-500  focus:ring-orange-500 rounded-md shadow-sm resize-y">{{ old('address') }}</textarea>
