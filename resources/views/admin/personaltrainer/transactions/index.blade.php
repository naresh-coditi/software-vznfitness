    ...
    <div class="flex items-center gap-4 px-4 border-b bg-white border-gray-300 pl-6 py-2 shadow-sm  text-xl font-bold">
        ...
        <div>
            ...
            <span class="block text-xs font-normal text-gray-500 mt-2">
                ...
                <a href="{{ route(auth()->user()->roleName . 'pt.index') }}">{{ __('Manage PT') }}</a> &raquo;
                <a>{{ __('Transaction') }}</a>
            </span>
        </div>
    </div>
...
@section('main-section')
    <section class="w-full px-6 mt-2 py-6">
        ...
        <div class="bg-white xl:w-2/3 m-auto w-full rounded-lg p-6">
            <form action="{{ route(auth()->user()->roleName . 'pt.transactions.store', $user) }}" method="post"
                class="gap-2">
                @csrf
                {{-- Method Type --}}
                ...
                <div class="sm:col-span-3">
                    ...
                    <x-text-input id="remaining_amount" class="block mt-1 w-full" type="text"
                        placeholder="Enter remianing amount" name="remaining_amount" :value="$user->latestPersonalTrainerPlan->remaining_balance ?? 'N/A'"
                ...
                <div class="gap-2 flex mt-6">
                    <a href="{{ route(auth()->user()->roleName . 'pt.index', $user) }}"
            ...
            <x-table-element>
                ...
                <tbody>
                    @forelse ($users as $key => $user)
                        @foreach ($user->latestTransactions as $transaction)
                            <x-table-row>
                                <x-table-data>
                                    <span>{{ $key += 1 }}</span>
                                </x-table-data>
                                <x-table-data>
                                    <span>{{ $transaction->transaction_date ?? 'N/A' }}</span>
                                </x-table-data>
                                <x-table-data>
                                ...
                                <x-table-data>
                                    <span>{{ $transaction->method_type ?? 'N/A' }}</span>
                                </x-table-data>
                                <x-table-data>
                                    <span>{{ $transaction->note ?? 'N/A' }}</span>
                                </x-table-data>
                                <x-table-data>
                                    <span>{{ $user->createdBy->userProfile->fullName ?? 'N/A' }}</span>
                                </x-table-data>
                                @can('isAdmin')
                                    <x-table-data class="flex gap-3">
                                        <a href="{{ route(auth()->user()->roleName . 'pt.transactions.edit', $transaction) }}"
                                        ...
                                        <x-delete-confirmation-modal :values="$transaction" :message="'Are you sure you want to remove this transaction ?'"
                                            routename="{{ route(auth()->user()->roleName . 'pt.transactions.delete', $transaction) }}" />
                                    </x-table-data>
                                @endcan
                            </x-table-row>
                        @endforeach
