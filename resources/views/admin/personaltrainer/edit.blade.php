...
@section('main-section')
    <section class="w-full px-6 mt-2 py-6">
        <div>
            <form action="{{ route(auth()->user()->roleName . 'pt.update', $data) }}" id="createUserForm" method="post">
                @csrf
                @method('PUT')
                        ...
                        <div class="grid max-w-2xl grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6 md:col-span-2">
                                ...
                                <select name="member" id="member" class="member"
                                    ...
                                    @foreach ($users as $user)
                                        <option value="{{ $user->id }}"
                                            {{ $user->id == $data->member_id ? 'selected' : '' }}>
                            ...
                            <div class="sm:col-span-3">
                                ...
                                <select name="trainer" id="trainer" class="trainer"
                                    ...
                                    <!-- if value of trainer not autofill it means trainer_id in null in PT -->
                            ...
                            <div class="sm:col-span-3">
                                ...
                                <x-text-input id="amount" class="block mt-1 w-full" type="number" name="amount"
                                    :value="$data->amount" required autofocus autocomplete="amount" />
                            ...
                            <div class="sm:col-span-3">
                                ...
                                <x-text-input id="remaining_balance" class="block mt-1 w-full" type="number"
                                    name="remaining_balance" :value="$data->remaining_balance" required autofocus
                                ...
                                <select name="method_type" id="method_type" required
                                    ...
                                    @foreach ($methods as $method)
                                        <option value="{{ $method['name'] }}"
                                            {{ $method['name'] == $data->method_type ? 'selected' : '' }}>
                                            {{ $method['name'] }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="sm:col-span-3">
                                ...
                                <select name="duration" id="duration"
                                    ...
                                    <option value="1 month" {{ $data->duration == '1 month"' ? 'selected' : '' }}>
                                        {{ __('1 month') }}</option>
                                    <option value="2 month" {{ $data->duration == '2 month"' ? 'selected' : '' }}>
                                        {{ __('2 month') }}</option>
                                    <option value="3 month" {{ $data->duration == '3 month"' ? 'selected' : '' }}>
                                        {{ __('3 month') }}</option>
                                    <option value="4 month" {{ $data->duration == '4 month"' ? 'selected' : '' }}>
                                        {{ __('4 month') }}</option>
                                    <option value="5 month" {{ $data->duration == '5 month"' ? 'selected' : '' }}>
                                        {{ __('5 month') }}</option>
                                    <option value="6 month" {{ $data->duration == '6 month"' ? 'selected' : '' }}>
                                        {{ __('6 month') }}</option>
                                    <option value="7 month" {{ $data->duration == '7 month"' ? 'selected' : '' }}>
                                        {{ __('7 month') }}</option>
                                    <option value="8 month" {{ $data->duration == '8 month"' ? 'selected' : '' }}>
                                        {{ __('8 month') }}</option>
                                    <option value="9 month" {{ $data->duration == '9 month"' ? 'selected' : '' }}>
                                        {{ __('9 month') }}</option>
                                    <option value="10 month" {{ $data->duration == '10 month"' ? 'selected' : '' }}>
                                        {{ __('10 month') }}</option>
                                    <option value="11 month" {{ $data->duration == '11 month"' ? 'selected' : '' }}>
                                        {{ __('11 month') }}</option>
                                    <option value="12 month" {{ $data->duration == '12 month"' ? 'selected' : '' }}>
                                        {{ __('12 month') }}</option>
                                </select>
                            ...
                            <div class="sm:col-span-3">
                                ...
                                <x-text-input id="start_date" class="block mt-1 w-full" type="date"
                                    placeholder="Enter start_date" name="start_date" :value="dateFormat($data->start_date, 'Y-m-d')" required autofocus
                            ...
                            <div class="sm:col-span-3">
                                ...
                                <x-text-input id="end_date" class="block mt-1 w-full" type="date"
                                    placeholder="Enter end_date" name="end_date" :value="dateFormat($data->end_date, 'Y-m-d')" required autofocus
                ...
                <div class="mt-6 flex items-center justify-end gap-x-6">
                    ...
                    <button type="submit"
                        class="rounded-md bg-orange-600 px-6 py-2 text-sm font-semibold text-white shadow-sm hover:bg-orange-500 focus-visible:outline-orange-500">Update</button>
                </div>
            </form>
        </div>
    </section>
