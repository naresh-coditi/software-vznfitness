...
@endpush
    ...
    @push('breadcrum')
        <div class="mb-4">
            <span class=" text-xl font-bold">{{ __('Trainers') }}</span>
    ...
    <section class="flex md:flex-row flex-col md:items-center gap-4 justify-between mb-4 ">
        ...
        <div class="text-right order-1 sm:order-2">
            <x-add-button-link content="ADD TRAINER"
                url="{{ route(auth()->user()->roleName . 'trainers.create') }}" />
        </div>
    </section>
        ...
        <tbody class="divide-y divide-gray-200 bg-white">
            @foreach ($trainers as $key => $trainer)
                <x-table-row>
                    ...
                    <x-table-data>
                        <span>{{ $trainer->userProfile->first_name . ' ' }}{{ $trainer->userProfile->last_name }}</span>
                    </x-table-data>
                    <x-table-data>
                    ...
                    <x-table-data>
                        <span>{{ $trainer->userProfile->experience }}</span>
                    </x-table-data>
                    <x-table-data>
                        <span>{{ dateFormat($trainer->created_at) }}</span>
                    </x-table-data>
                    <x-table-data>
                        <span>{{ $trainer->createdBy->userProfile->fullName }}</span>
                    </x-table-data>
                    <x-table-data class="relative">
                        <x-modal.action-modal class="-left-32">
                            <li>
                                <a href="{{ route(auth()->user()->roleName . 'trainers.view', $trainer) }}"
                            ...
                            <li>
                                <a href="{{ route(auth()->user()->roleName . 'trainers.edit', $trainer) }}"
                                    class="cursor-pointer hover:text-blue-500 flex items-center gap-2">
                            ...
                            <li>
                                <x-delete-confirmation-modal label :values="$trainer" :message="'Are you sure you want to remove this trainer ?'"
                                    routename="{{ route(auth()->user()->roleName . 'trainers.delete', $trainer) }}" />
                            </li>
    ...
    <div class="py-3">{{ $trainers->links() }}</div>
    @endsection
