    ...
    <section class="w-full px-6 mt-2">
        <div class="flex items-center justify-between">
            ...
            <div class="w-full text-right mb-2">
                <x-add-button-link content="Add Plan"
                    url="{{ route(auth()->user()->roleName . 'workout.plans.create') }}" />
            </div>
        </div>
        <x-table-element>
            ...
            <tbody class="divide-y divide-gray-200 bg-white">
                @forelse ($plans as $key => $plan)
                    <x-table-row>
                        ...
                        <x-table-data>
                            <span>{{ $plan->createdBy->userProfile->fullName }}</span>
                        </x-table-data>
                        <x-table-data>
                            <span>{{ dateFormat($plan->created_at) }}</span>
                        </x-table-data>
                        {{-- Action --}}
                        <x-table-data class="relative">
                            <x-modal.action-modal class="-left-32">
                                <li>
                                    <a href="{{ route(auth()->user()->roleName . 'workout.plans.view', $plan) }}"
                                ...
                                <li>
                                    <a href="{{ route(auth()->user()->roleName . 'workout.plans.edit', $plan) }}"
                                        class="cursor-pointer hover:text-blue-500  flex items-center gap-2">
                                ...
                                <li>
                                    <x-delete-confirmation-modal label :values="$plan" :message="'Are you sure you want to remove this category ?'"
                                        routename="{{ route(auth()->user()->roleName . 'workout.plans.delete', $plan) }}" />
                                </li>
