    ...
    <section class="w-full px-6 mt-2" x-data="{}">
        <div class="flex items-center justify-between">
            ...
            <div class="w-full text-right mb-2">
                <x-add-button-link content="Add Exercise" url="{{ route(auth()->user()->roleName . 'exercises.create') }}" />
            </div>
        </div>
        <x-table-element>
            ...
            <tbody class="divide-y divide-gray-200 bg-white">
                @forelse ($exercises as $key => $exercise)
                    <x-table-row>
                        <x-table-data>
                            <span>{{ $key + $exercises->firstItem() }}</span>
                        </x-table-data>
                        <x-table-data>
                        ...
                        <x-table-data>
                            <span>{{ $exercise->createdBy->userProfile->fullName }}</span>
                        </x-table-data>
                        <x-table-data>
                            <span>{{ dateFormat($exercise->created_at) }}</span>
                        </x-table-data>
                        {{-- Action --}}
                        <x-table-data class="relative">
                            <x-modal.action-modal class="-left-36 pr-4">
                                <li>
                                    <a href="{{ route(auth()->user()->roleName . 'exercises.upload.create', $exercise) }}"
                                ...
                                <li>
                                    <a href="{{ route(auth()->user()->roleName . 'exercises.edit', $exercise) }}"
                                        class="cursor-pointer hover:text-blue-500  flex items-center gap-2">
                                ...
                                <li>
                                    <x-delete-confirmation-modal label :values="$exercise" :message="'Are you sure you want to remove this exercise ?'"
                                        routename="{{ route(auth()->user()->roleName . 'exercises.delete', $exercise) }}" />
                                </li>
        ...
        <div class="py-3">{{ $exercises->links() }}</div>
